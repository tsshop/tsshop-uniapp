<template>
	<view :class="'interlayer '+(c||'')" :style="s">
		<block v-for="(n, i) in nodes" v-bind:key="i">
			...
			<view v-if="n.name=='img'" :class="'_img '+n.attrs.class" :style="{width:'100%',height:'auto','verticalAlign':'top'}" :data-attrs="n.attrs"
				@tap.stop="imgtap">
				<rich-text v-if="ctrl[i]!=0"
					:nodes="[{attrs:{src:loading&&(ctrl[i]||0)<2?loading:(lazyLoad&&!ctrl[i]?placeholder:(ctrl[i]==3?errorImg:n.attrs.src||'')),alt:n.attrs.alt||'',width:n.attrs.width||'',style:'vertical-align: top !important;-webkit-touch-callout:none;max-width:100%;display:block'+(n.attrs.height?';height:'+n.attrs.height:'')},name:'img'}]" />
				<image class="_image" :src="lazyLoad&&!ctrl[i]?placeholder:n.attrs.src" :lazy-load="lazyLoad"
					...
					@load="loadImg" @error="error" style="font-size: 0 !important;vertical-align: top !important;"/>
			</view>
			<!--文本-->
			...
			<view v-else-if="n.name=='a'" :id="n.attrs.id" :class="'_a '+(n.attrs.class||'')" hover-class="_hover"
				:style="n.attrs.style" :data-attrs="n.attrs" @tap.stop="linkpress">
			...
			<view v-else-if="n.name=='table'&&n.c&&n.flag" :id="n.attrs.id" :class="n.attrs.class"
				...
				<trees v-for="(cell,n) in n.children" v-bind:key="n" :class="cell.attrs.class" :c="cell.attrs.class"
					:style="cell.attrs.style" :s="cell.attrs.style" :nodes="cell.children" />
			</view>
