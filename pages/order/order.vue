	...
	<view>
		...
		<view v-for="(item,index) in list" :key="index" class="ml-31 w-688 mt-20 br-16 bg-FFFFFF order-item">
			...
			<view class="mt-20 flex flex-x-end">
				...
				<view class="order-btn pay" v-if="item.orderStatus == 0" @tap.stop="pay_2(item)">立即支付</view>
	...
	export default {
		...
		methods: {
			...
			pay_2(item) {
				...
				//选择支付方式
				if (!item.id) return //如果没有id
				this.$refs.payList.open({
					price: item.totalAmount, //价格
					success: (id, price, detail) => {
						...
						start_pay({
							...
							success: () => {
								this.showToast('支付成功');
								getApp().globalData.payOrderId = item.id;
								setTimeout(() => {
